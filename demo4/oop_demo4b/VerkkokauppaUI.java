...
public class VerkkokauppaUI {
    ...
    public VerkkokauppaUI() {
        ...
        ArrayList<Asiakas> asiakkaat = VerkkokauppaIO.lueAsiakkaat("asiakkaat.txt");
        ArrayList<Myyja> myyjat = VerkkokauppaIO.lueMyyjat("myyjat.txt");
        ArrayList<Tuote> tuotteet = VerkkokauppaIO.lueTuotteet("tuotteet.txt");
        ArrayList<Ostotapahtuma> ostotapahtumat = VerkkokauppaIO.lueOstotapahtumat("ostotapahtumat.txt");

        if(asiakkaat.size() > 0){
            asiakkaat.forEach(asiakas -> verkkokauppa.lisaaAsiakas(asiakas));
        }
        if(myyjat.size() > 0){
            myyjat.forEach(myyja -> verkkokauppa.lisaaMyyja(myyja));
        }
        if(tuotteet != null) {
            tuotteet.forEach(tuote -> verkkokauppa.lisaaTuote(tuote));
        }
        if(ostotapahtumat != null) {
            ostotapahtumat.forEach(tapahtuma -> verkkokauppa.lisaaTapahtuma(tapahtuma));
        }
    ...
    public void aloita() {
        ...
        VerkkokauppaIO.kirjoitaAsiakkaat(verkkokauppa.getAsiakkaat(), "asiakkaat.txt");
        VerkkokauppaIO.kirjoitaMyyjat(verkkokauppa.getMyyjat(), "myyjat.txt");
        VerkkokauppaIO.kirjoitaTuotteet(verkkokauppa.getTuotteet(), "tuotteet.txt");
        VerkkokauppaIO.kirjoitaOstotapahtumat(verkkokauppa.getTapahtumat(), "ostotapahtumat.txt");
    }
    ...
    private void ostotapahtumaMenu() {
        ...
        while(valinta != 0) {
            ...
            if(valinta == 1){
                ...
                System.out.println(verkkokauppa.listaaAsiakkaat());
                System.out.println();
                System.out.println("Valiste asiakas");
                String asiakasnumero = lueMerkkijono("Asiakasnumero");
                int maara = lueKokonaisluku(1, verkkokauppa.annaTuote(tuoteNimi).getSaldo(), "Määrä");
                ...
                if(asiakas instanceof KantaAsiakas){
                    hinta = maara * tuote.getHinta() * (asiakas.getAlennusprosentti() / 100.0);
                } else {
                    hinta = maara * tuote.getHinta();
                }

                verkkokauppa.lisaaTapahtuma(new Ostotapahtuma(asiakas, myyja, tuote, maara));

                tuote.setSaldo(tuote.getSaldo() - maara);
                myyja.lisaaProvisio(hinta * 0.10);
                asiakas.lisaaOsto(hinta);
            } else if (valinta == 2){
                System.out.println(verkkokauppa.listaaTapahtumat());
                System.out.println();
                int poistettava = lueKokonaisluku(1, verkkokauppa.getTapahtumatMaara() + 1, "Poistettava");
                verkkokauppa.poistaOstotapahtuma(verkkokauppa.annaTapahtuma(poistettava));
            }
        }
    }
